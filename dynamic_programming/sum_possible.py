...
def sum_possible(amount, numbers, memo={}):
    ...
    for num in numbers: # loop to test all nums in numbers
      new_num = amount - num
      if sum_possible(new_num, numbers, memo): # if the function call of sum_possible with the new number == True, assign the value of True to the key amount in memo and return true for the function call
        memo[amount] = True
        return True
    memo[amount] = False
    return False 
